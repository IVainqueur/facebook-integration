...
const SideBar = () => {
    ...
    const { isLoggedIn, logAction, userInfo } = useContext<FacebookCredentials>(facebookCredentialsContext)
        ...
        <div className={`bg-sky-700 h-screen overflow-x-hidden ${showSideBar ? 'w-[350px]' : 'w-0'} grid  text-white place-content-center `}>
            {
                isLoggedIn ?
                    <div className="flex flex-col items-center h-fit gap-4 -mt-32">
                        ...
                        <img
                            src={`${userInfo?.id ? `https://graph.facebook.com/${userInfo.id}/picture?type=normal` : 'https://picsum.photos/200'}`}
                            alt="Profile pic"
                            className="h-32 w-32 rounded-full bg-transparent border-dashed border-2 border-slate-200"
                        />
